//! LEGACY (v1) — quarantined in S1 (#32). V1 deliberate-practice generation
//! and scheduling. Replaced by S9: mastery model + deliberate practice
//! retarget (#40). Do not extend. Deleted in S17 (#48).

use crate::legacy::db::Db;
use crate::legacy::generate::extract_complete_items;
use crate::legacy::session::SessionItem;
use async_openai::{
    config::OpenAIConfig,
    types::{
...
fn persist_dp_item(conn: &Connection, item: &crate::legacy::generate::GeneratedItem) -> rusqlite::Result<()> {
    let id = dp_uuid();
