...
- **Exercise generation** is performed lazily per unit, in the background, and cached. The first time a unit is opened (unit detail screen), a single whole-unit generation call is initiated; the resulting items are persisted to the exercise bank. Every subsequent session for that unit reads from the bank — generation is never invoked at runtime mid-session. There is no upfront pilot bank and no human review step in the loop. The whole-unit batch principle is preserved (one call, one context — coherence, no duplicates, correct difficulty curve and stack ratio); item-level on-demand generation is explicitly rejected. Performance optimizations applied:
  - **Streaming + incremental persistence:** the generation response is streamed and items are committed to the bank as they arrive, so the practice session can begin on item 1 while later items are still generating. The session queue assembler reads whatever is currently in the bank.
  - **Adjacent-unit prefetch:** when a unit is opened (or when the home screen resolves "Continue"), the next curriculum unit's bank is generated in the background if empty, so subsequent units feel instant.
  - **Prompt caching on the stable prefix:** the generation prompt is structured so the style guide, schema, and shared instructions form a cacheable prefix; only per-unit variables change. This reduces input tokens and time-to-first-token on back-to-back unit generations.
  - **App-open pre-warm sweep:** on launch, units in "in progress" state or the next-likely-next unit with empty banks are filled in the background.
- **Generation failure UX:** if the initial generation for a unit fails, the unit detail screen shows a retry CTA and disables Start until the bank is populated. Background prefetch failures are silent and retried on the next trigger. Regeneration of an already-populated unit is a manual/admin action.
...
- LLM provider is **OpenAI** (model TBD at implementation). Tauri version and frontend framework must be finalized before implementation begins.
